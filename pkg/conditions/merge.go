...
func merge(conditions []localizedCondition, targetCondition av1.KubedgeConditionType, options *mergeOptions) *av1.KubedgeCondition {
	g := getConditionGroups(conditions)
	if len(g) == 0 {
		return nil
	}

	if g.TopGroup().status == av1.ConditionStatusTrue {
...
func getConditionGroups(conditions []localizedCondition) conditionGroups {
	...
	for _, condition := range conditions {
		if condition.KubedgeCondition == nil {
			continue
		}
	...
	if len(groups) > 0 {
		sort.Slice(groups[0].conditions, func(i, j int) bool {
			...
			if a.Type != b.Type {
				return lexicographicLess(a.KubedgeCondition, b.KubedgeCondition)
			}
...
func (g conditionGroups) TrueGroup() *conditionGroup {
	return g.getByStatusAndSeverity(av1.ConditionStatusTrue, av1.ConditionSeverityNone)
}
...
func (g conditionGroups) ErrorGroup() *conditionGroup {
	return g.getByStatusAndSeverity(av1.ConditionStatusFalse, av1.ConditionSeverityError)
}
...
func (g conditionGroups) WarningGroup() *conditionGroup {
	return g.getByStatusAndSeverity(av1.ConditionStatusFalse, av1.ConditionSeverityWarning)
}

func (g conditionGroups) getByStatusAndSeverity(status av1.KubedgeConditionStatus, severity av1.KubedgeConditionSeverity) *conditionGroup {
	if len(g) == 0 {
		return nil
	}
	for _, group := range g {
...
type conditionGroup struct {
	status     av1.KubedgeConditionStatus
	severity   av1.KubedgeConditionSeverity
...
func (g conditionGroup) mergePriority() int {
	switch g.status {
	case av1.ConditionStatusFalse:
		switch g.severity {
		case av1.ConditionSeverityError:
			return 0
		case av1.ConditionSeverityWarning:
			return 1
		case av1.ConditionSeverityInfo:
			return 2
		}
	case av1.ConditionStatusTrue:
		return 3
	case av1.ConditionStatusUnknown:
		return 4
	}
