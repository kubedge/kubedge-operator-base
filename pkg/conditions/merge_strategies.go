...
type mergeOptions struct {
	conditionTypes                     []av1.KubedgeConditionType
	addSourceRef                       bool
	addStepCounter                     bool
	addStepCounterIfOnlyConditionTypes []av1.KubedgeConditionType
	stepCounter                        int
}
...
func WithConditions(t ...av1.KubedgeConditionType) MergeOption {
...
// IMPORTANT: This options works only while generating the Summary condition.
func WithStepCounterIfOnly(t ...av1.KubedgeConditionType) MergeOption {
	return func(c *mergeOptions) {
...
func getReason(groups conditionGroups, options *mergeOptions) av1.KubedgeConditionReason {
...
func getFirstReason(g conditionGroups, order []av1.KubedgeConditionType, addSourceRef bool) av1.KubedgeConditionReason {
	...
	return av1.KubedgeConditionReason("")
}

// localizeReason adds info about the originating object to the target Reason.
func localizeReason(reason av1.KubedgeConditionReason, from Getter) av1.KubedgeConditionReason {
	if strings.Contains(reason.String(), "@") {
		return reason
	}
	return av1.KubedgeConditionReason(fmt.Sprintf("%s @ %s/%s", reason, from.GetObjectKind().GroupVersionKind().Kind, from.GetName()))
}
...
func getFirstMessage(groups conditionGroups, order []av1.KubedgeConditionType) string {
...
func getFirstCondition(g conditionGroups, priority []av1.KubedgeConditionType) *localizedCondition {
	topGroup := g.TopGroup()
	if topGroup == nil {
		return nil
	}
